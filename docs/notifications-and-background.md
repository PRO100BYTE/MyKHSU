...
Устаревший дублирующий модуль `utils/newsNotifications.js` удалён: единственный рабочий поток уведомлений о новостях проходит через `backgroundService` и `notificationService`.
...
- задача всегда запрашивает свежую ленту, обходя TTL-кэш, но при ошибке сети не отправляет уведомления на основании устаревшего кэша;
- первая успешная проверка только сохраняет baseline последних новостей, чтобы не уведомлять о старой ленте;
- последующие проверки сравнивают устойчивые ключи «дата + фрагмент текста», отправляют не более трёх локальных уведомлений и обновляют baseline даже при отсутствии изменений;
- ошибки обрабатываются безопасно, без падения приложения.

## Платформенные ограничения

- Фоновые задачи могут вести себя по-разному на Android и iOS. `minimumInterval` — только пожелание ОС: Android может отложить задачу из-за энергосбережения, а iOS запускает её эвристически и не гарантирует выполнение после принудительного закрытия приложения.
- Локальные фоновые уведомления работают на обеих платформах, но не являются заменой удалённым push-уведомлениям для срочных новостей.
- Для гарантированной доставки на iOS и Android при закрытом приложении нужен сервер: он должен отслеживать новости и отправлять Expo Push Notifications / APNs / FCM. Это отдельная backend-задача с хранением push-токенов и согласием пользователя.

...
- pull-to-refresh расписания всегда запрашивает свежие данные с сервера; при сетевой ошибке сохраняется безопасный fallback на кэш;
- ошибки загрузки времени пар (`fetch pairs time`) обрабатываются в том же общем error-flow, что и остальные ошибки расписания;
...
- fallback на кэш;
- при отсутствии сети или недоступности API показывается общий экран ошибки с кнопкой перехода к кэшированным новостям (если кэш есть);
- синхронизация cache-статуса с хедером.
- поддержка даты новости в формате `YYYY-MM-DD HH:mm:ss` (например, `2026-04-22 07:50:10`) наряду с ISO-форматом.
...
- отображение интерактивной карты и маркеров зданий на iOS;
- фильтры категорий;
- список корпусов;
- открытие вариантов маршрута во внешних сервисах;
- на Android интерактивная карта намеренно отключена из-за ограничений конфигурации Google Maps API: вместо неё показывается понятное fallback-состояние со списком корпусов и маршрутизацией;
...
- режим разработчика с инструментами тестирования (комплексный smoke-test, пинг API, проверка permissions уведомлений, инспектор ключей AsyncStorage, тесты и массовая разблокировка достижений);
- кастомный API-эндпоинт в меню разработчика применяется сразу после сохранения (без перезапуска);
- API-кэш разделяется по endpoint, чтобы при переключении сервера не подхватывались данные от предыдущего адреса.
- служебные действия, включая очистку API-кэша без удаления личных данных (заметок, ДЗ, посещаемости, избранного, достижений и учебных событий) и отдельную очистку заметок.

### Единый стиль подразделов настроек

Подразделы, открываемые из экрана настроек, приведены к единому визуальному паттерну:

- секционные заголовки через `SectionHeader`;
- группировка контента через `SettingsGroup`;
- единые компактные отступы, скругления, бордеры и тени карточек (визуально в ритме основного экрана настроек).

Базовые переиспользуемые UI-компоненты вынесены в `components/SettingsComponents.js` и применяются в подразделах:
...
Подраздел `DeveloperMenuScreen` также приведен к тем же правилам секционирования (единый стиль заголовков секций и карточных блоков).
...
## Экран «Студгородок» (`components/FreshmanScreen.js`)

Экран с полезными материалами для студентов, сгруппированными по категориям для быстрого доступа.

### Возможности раздела «Студгородок»

- быстрые ссылки на ключевые ресурсы ИТИ/ХГУ;
- список корпусов университета;
- полезные группы и сообщества;
- раздел «Полезные группы (сообщества)» структурирован по платформам (ВКонтакте, Telegram, MAX);
- локальный экран «Сообщество ИТИ»: черновики материалов и анонимные отзывы о преподавателях сохраняются только на устройстве; обмен между студентами будет добавлен после появления backend, авторизации и модерации;
- расписание звонков с сервера;
- календарь учебных событий с экспортом в календарь и ICS;
- словарь аббревиатур;
- категоризация главного экрана по блокам «Учеба», «Ресурсы и сервисы», «Кампус и сообщества»;
- вложенные представления с корректным возвратом назад;
- обработка ошибок загрузки расписания звонков через `ConnectionError` с кнопкой повтора.
...
## Запланированные возможности, требующие нативной или серверной части

- Виджеты «текущая/следующая пара» для Android и iOS требуют нативных модулей (App Widget / WidgetKit), `expo prebuild` и проверки в development или production-сборках. В managed Expo без нативной настройки их безопасно добавить нельзя.
- Надёжная доставка новостей при полностью закрытом приложении требует backend-уведомлений через Expo Push Notifications, APNs и FCM; локальная фоновая задача остаётся best-effort-механизмом.
- Расширенная статистика учебной нагрузки и офлайн-предзагрузка семестра требуют согласования формата данных API и лимитов локального хранилища.
- Личные достижения, streak и локальные напоминания уже работают на устройстве; общий рейтинг намеренно не реализуется без сервера и защиты от накруток.
...
- компактная секционная подача в стиле остальных подразделов настроек.
...
## Общий экран ошибки загрузки (`components/ConnectionError.js`)

Для экранов расписания, карты и новостей используется единый компонент ошибки загрузки с параметрами экрана.

Минимальный контракт:

- `screen`: идентификатор экрана (`schedule`, `map`, `news`, `general`);
- `errorType`: тип ошибки (например, `NO_INTERNET`, `API_UNAVAILABLE`, `load-error`);
- `icon`: имя иконки Ionicons для конкретного сценария;
- `title`: заголовок ошибки;
- `description`: описание ошибки для пользователя;
- `loading`: флаг состояния загрузки;
- `onRetry`: обработчик повторной попытки загрузки.

Поведение:

- компонент нормализует коды ошибок (`NO_INTERNET` -> `no-internet`, `API_UNAVAILABLE` -> `api-unavailable`);
- компонент обрабатывает ошибки формата ответа (`INVALID_JSON`, `SyntaxError: JSON Parse error: Unexpected character: *`) и показывает спокойное сообщение с рекомендацией попробовать снова сейчас или чуть позже;
- при отсутствии части параметров применяются fallback-конфиги по экрану и типу ошибки;
- если для раздела доступны кэшированные данные, показывается отдельная кнопка перехода к кэшу;
- рендер состояний загрузки и ошибки остается унифицированным для всех экранов.
